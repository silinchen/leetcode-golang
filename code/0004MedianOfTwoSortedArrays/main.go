 ...
 * 计算中位数：假设 A 集合有 n 个数值
 *    奇数：A 中的第 (n+1)/2 个元素，也就是最中间的那个数作为中位数
 *    偶数：A 中的 第 n/2 和 n/2+1 个元素，也就是最中间的那两个数的和除以2得到的值作为中位数
 * 例如：
 *    [1 2 3] 的中位数是 2
 ...
 * 已知奇数的时候，中位数是第 (n+1)/2 位，k 为 (n+1)/2，
 * 偶数的时候，是第 n/2 和 n/2+1 位 的和除以2，可以找 k 为 n/2+1，再找 k - 1
 *
 * 因为 n 是整数，所以 (n+1)/2 和 n/2+1 可以统一为 int(n/2)+1
 *
 * 例如：
 * [1 2 3] 的长度 n 是 3，k = int(3/2) + 1 = 2，中位数就是 A 的第二个元素 A[1] = 2。（数组下标从0开始，所以第2个数的下标是 1）
 * [1 2 3 4] 的长度 n 是 4，k = int(4/2) + 1 = 3，中位数就是 (A[1] + A[2])/2 = (2+3)/2 = 2.5
 *
